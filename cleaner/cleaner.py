...
from helpers import unicode_remover, punctuation_remover, character_transformer, contraction_transformer, lemmatize, save_wordcloud, save_tfidf
...
class Cleaner():
    ...
    def save_files(self, directory, file_type, callable_name, mask_path, restaurant_ids='all'):
        """ Saves files (Wordclouds or TF-IDF) for corpora """
        
        logger.warn(f' > SAVING {file_type} FILES ')
        ...
        if restaurant_ids == 'all':
            for restaurant_id, df in self.df_word_frequency.items():
                callable_name(df, self.filename, restaurant_id, directory, np.array(Image.open(mask_path)))
        else:
